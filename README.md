...
- **TB_REVIEW Entity**
  ...
  - 회원 테이블의 `userId`와 영화 테이블의 `movieCode`, 예매 내역 테이블의 `TicketNo`를 FK로 가지며 한 명의 사용자는 영화 하나 당 하나의 리뷰만 작성할 수 있습니다. 

- **TB_OPEN_TALK Entity**
  - 여러 명의 사용자들이 대화를 할 수 있는 오픈톡 테이블입니다. 
  - `talkId`를 PK로 가지고 TB_MEMBER테이블의 `userId`를 FK로 가집니다.

- **TB_NOTICE Entity**
  - 관리자가 공지사항을 등록할 수 있는 공지사항 테이블입니다. 
  - `noticeNo`를 PK로, TB_MEMBER테이블의 `userId`를 FK로 설정하였습니다.
